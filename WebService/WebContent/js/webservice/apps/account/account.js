...
function accountBalance(){
	 ...
	 * 账目统计报表
	 */
	this.accountReport = function(url){
	...
	function getRestoreForm(uri){
		...
		var importForm = new Ext.form.FormPanel({
			url: path + "/account_manage/uploadAccount.action?method=uploadAccount",
			title:"导入账目信息",
			frame: true,
			labelAlign: 'right',
			labelWidth:70,
			   ...
			   new Ext.Button({
				   ...
				   handler:function(){
					   if(importForm && importForm.form.isValid()){
					       ///account_manage/uploadAccount.action?method=uploadAccount
					       doImportAccount("accountBackupWindow", importForm);
					   }
				   }
			   }),"-",new Ext.Button({
                   text:"下载账目文件模板",
                   ...
                   handler:function(){
                       ...
                       dform.action = path + "/account_manage/downloadTemplate.action?method=download";
                       dform.method ="post";
                       dform.submit();
                   }
               }),
	 ...
	 * @param form
	 */
	function doImportAccount(windowId, form){
	    Ext.MessageBox.show({
            msg:"正在上传账目信息，请稍候...",
            progressText:"正在上传账目信息，请稍候...",
            width:300,
            wait:true,
            waitConfig: {interval:200},
            icon:Ext.Msg.INFO
        ...
        form.getForm().submit({
            ...
            success: function(form, action) {
                ...
                try{
                    ...
                    if(result && result.success){
                        ...
                        var msg = "账目信息上传失败，请检查您所填信息是否完整无误！";
                        if(result.msg){
                            msg = result.msg;
                        }
            ...
            failure: function(form, action) {//action.result.errorMessage
                Ext.Msg.hide();
                var msg = "账目信息上传失败，请检查您的网络连接或者联系管理员！";
                try{
                    var result = Ext.decode(action.response.responseText);
                    if(result.msg){
                        msg = result.msg;
