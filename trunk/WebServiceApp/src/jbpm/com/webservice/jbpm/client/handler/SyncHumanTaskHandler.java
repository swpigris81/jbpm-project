 ...
 * 
 */
public class SyncHumanTaskHandler implements WorkItemHandler {

    private static final Logger logger = LoggerFactory.getLogger(SyncHumanTaskHandler.class);
    ...
    public SyncHumanTaskHandler() {
        this.action = OnErrorAction.LOG;
    }

    public SyncHumanTaskHandler(TaskService client) {
        this.client = client;
        this.action = OnErrorAction.LOG;
    }
    
    public SyncHumanTaskHandler(TaskService client, KnowledgeRuntime session) {
        this.client = client;
        this.session = session;
        this.action = OnErrorAction.LOG;
    }
    
    public SyncHumanTaskHandler(TaskService client, OnErrorAction action) {
        this.client = client;
        this.action = action;
    }
    
    public SyncHumanTaskHandler(TaskService client, KnowledgeRuntime session, OnErrorAction action) {
        this.client = client;
        this.session = session;
        this.action = action;
    }
    ...
    private class TaskCompletedHandler extends AbstractBaseResponseHandler implements EventResponseHandler {
        ...
        public void handleCompletedTask(long taskId) {
            ...
            if (task.getTaskData().getStatus() == Status.Completed) {
                ...
                if (contentId != -1) {
                    ...
                    Object result = null;
                    if(content != null){
                        result = readContentData(content);
                    }
        ...
        /**
         * <p>Discription:[解析结果数据]</p>
         * @param content
         * @return
         * @author 大牙-小白
         * @update 2012-9-6 大牙-小白 [变更描述]
         */
        private Map<String, Object> readContentData(Content content){
